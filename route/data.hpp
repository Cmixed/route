...
namespace route
{
    ...
    private:
        static std::vector<std::vector<int>> initialize_population(int const start, int const end, int const vertices, int const population_size)
        {
        ...
        static int calculate_path_distance(const std::vector<int>& path, const std::vector<std::vector<int>>& adj_matrix)
        {
            int distance = 0;
            for (size_t i = 0; i < path.size() - 1; ++i) {
